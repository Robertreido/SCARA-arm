...
public class ToolPath
{
    ...
    public void convert_drawing_to_angles(Drawing drawing,Arm arm,String fname){
        ...
        for (int i = 1;i < drawing.get_drawing_size();i++){ 
            ...
            //calculate n_steps
            n_steps = (int) (Math.sqrt(Math.pow(p0.get_x()-p1.get_x(),2)+Math.pow(p0.get_y()-p1.get_y(),2))/2);
    ...
    public void save_pwm_file(String fname){
        for ( int i = 0 ; i < pwm1_vector.size(); i++){
         ...
         try {
            ...
            //write the first point twice with the pen up so it moves to that point
            //write the last line to pick the pen up
            str_out = String.format("%d,%d,%d\n", pwm1_vector.get(1),pwm2_vector.get(1),1300);
            w.write(str_out);
            ...
            //write the last line to pick the pen up
            str_out = String.format("%d,%d,%d\n", pwm1_vector.get(pwm1_vector.size()-1),pwm2_vector.get(pwm1_vector.size()-1),1300);
            w.write(str_out);
